...
1. 基本算术运算符  
  除法运算符/中，两个实数相除的结果是双精度实数，如果除数或被除数中有一个为负值，多数C编译系统采取“向零取整”的方法。
2. 自增自减运算符  
  前置：先自增（减），再使用。  
  后置：先使用，再自增（减）。
3. 算术表达式和运算符的优先级和结合性  
  ...
  结合性：左结合、右结合
4. 混合运算  
  $float和double\longrightarrow double$  
  $ int、float和double\longrightarrow double$  
  $char和整型\longrightarrow 整型、char和实型double$  
5. 强制类型转换运算符  
  (类型名)(表达式)  
  a=(double)x  
  **注意：x的值和类型都未变化。**
